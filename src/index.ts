...
function helpMessage(): AgnosticReplyOptions {
        ...
        const embed = new Discord.EmbedBuilder()
                ...
                .setFooter({
                        text: `${packageJson().name} ${getVersion()} by ${(packageJson().author as PackageJsonPerson).name
                                }`,
                })
        return {
                embeds: [embed],
...
client.on('messageCreate', async (message) => {
        ...
        switch (command) {
                case "help": await message.channel.send(helpMessage())
                case "invite": await message.channel.send(inviteMessage())
                case "train": {
        ...
        if (!command) {
                ...
                // Random change to reply 1 in 15
                const rand = Math.floor(Math.random() * 15) + 1
                if (rand === 0 || (client.user && message.mentions.has(client.user))) {
                ...
                if (await isValidChannel(message.channel)) {
                        ...
                        if (isBadWord(message.content.toLowerCase())) {
                                L.debug("Bad word detected, ignoring message")
                                return
                        }
